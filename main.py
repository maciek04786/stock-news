import requests
from datetime import datetime, timedelta
from twilio.rest import Client

STOCK = "TSLA"
COMPANY_NAME = "Tesla Inc"

today = datetime.now()
yesterday = today.date() - timedelta(days=1)
yesterday_str = str(yesterday)
day_before_str = str(yesterday - timedelta(days=1))

# STEP 1: Use https://www.alphavantage.co
# When STOCK price increase/decreases by 5% between yesterday and the day before yesterday then print("Get News").

alpha_key = "YOUR_ALPHA_VANTAGE_KEY"

stock_params = {
    "function": "TIME_SERIES_DAILY",
    "symbol": STOCK,
    "interval": "daily",
    "apikey": alpha_key,
    "series_type": "close"
}

url = 'https://www.alphavantage.co/query'
stock_request = requests.get(url, params=stock_params)
stock_data = stock_request.json()["Time Series (Daily)"]

yesterday_price = float(stock_data[yesterday_str]["4. close"])
day_before_price = float(stock_data[day_before_str]["4. close"])

difference_perc = (yesterday_price - day_before_price) / day_before_price * 100.0

up_down = None
if difference_perc > 0:
    up_down = "🔺"
else:
    up_down = "🔻"

trigger_sms = False
if abs(difference_perc) >= 5:
    trigger_sms = True

# STEP 2: Use https://newsapi.org
# Instead of printing ("Get News"), actually get the first 3 news pieces for the COMPANY_NAME.

news_key = "YOUR_NEWS_API_KEY"

news_params = {
    "q": COMPANY_NAME,
    "apiKey": news_key,
    "sortBy": "publishedAt",
}

news_request = requests.get(url="https://newsapi.org/v2/everything", params=news_params)
news_data = news_request.json()

latest_news = news_data["articles"][:3]

# STEP 3: Use https://www.twilio.com
# Send a separate message with the percentage change and each article's title and description to your phone number.

account_sid = "YOUR_TWILIO_ACCOUNT_SID"
auth_token = "YOUR_TWILIO_AUTH_TOKEN"
if trigger_sms:
    for article in latest_news:
        client = Client(account_sid, auth_token)

        message = client.messages.create(
            to="RECIPIENT_NUMBER",
            from_="YOUR_TWILIO_NUMBER",
            body=f"{COMPANY_NAME}: {up_down}{abs(difference_perc):.2f}%\n"
                 f"Headline: {article['title']}\n"
                 f"Brief: {article['description']}"
        )

# Optional: Format the SMS message like this:

# TESLA: 🔺2%
# Headline: Were Hedge Funds Right About Piling Into Tesla Inc. (TSLA)?.
# Brief: We at Insider Monkey have gone over 821 13F filings that hedge funds and prominent investors are required,
#  to file by the SEC The 13F filings show the funds' and investors' portfolio positions as of March 31st,
#   near the height of the coronavirus market crash.
# or
# "TSLA: 🔻5%
# Headline: Were Hedge Funds Right About Piling Into Tesla Inc. (TSLA)?.
# Brief: We at Insider Monkey have gone over 821 13F filings that hedge funds and prominent investors are required,
#  to file by the SEC The 13F filings show the funds' and investors' portfolio positions as of March 31st,
#   near the height of the coronavirus market crash.
